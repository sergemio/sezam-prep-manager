...
- **3 dropdowns user** → 1 composant partagé : PM `toggleUserDropdown` (~L2090) + `toggleModalUserDropdown` (~L2123) + I&C inline dans `setupNavigation` (~L1222-1268).
- **2 `showStaffSelectionModal`** (PM prep-check ~L1781-1995 + I&C ~L1423-1527) → 1 modal partagé.
...
- **4 systèmes de tableaux** (`.inventory-table`, `.db-table`, `.overview-table`, `.data-table` — ce dernier écrit mais jamais branché) → 1 `.data-table`. Migrer le markup PUIS supprimer les anciennes classes.
